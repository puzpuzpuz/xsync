...
func TestIndexOfFirstMarkedByte(t *testing.T) {
	testCases := []struct {
		input    uint64
		expected int
	}{
	...
	for _, tc := range testCases {
		t.Run(strconv.Itoa(int(tc.input)), func(t *testing.T) {
			if IndexOfFirstMarkedByte(tc.input) != tc.expected {
				t.Errorf("unexpected result: %x", IndexOfFirstMarkedByte(tc.input))
			}
		})
	}
}
...
func TestZeroByte(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			word:     0xffffffffffffffff,
			idx:      1,
			expected: 0xffffffffffff00ff,
		},
		{
			word:     0xffffffffffffffff,
			idx:      2,
			expected: 0xffffffffff00ffff,
		},
		{
			word:     0xffffffffffffffff,
			idx:      3,
			expected: 0xffffffff00ffffff,
		},
		{
			word:     0xffffffffffffffff,
			idx:      4,
			expected: 0xffffff00ffffffff,
		},
		{
			word:     0xffffffffffffffff,
			idx:      5,
			expected: 0xffff00ffffffffff,
		},
		{
			word:     0xffffffffffffffff,
			idx:      6,
			expected: 0xff00ffffffffffff,
		},
		{
			word:     0xffffffffffffffff,
			idx:      7,
			expected: 0x00ffffffffffffff,
		},
		{
			word:     0,
			idx:      1,
		...
		},
		{
			...
			idx:      2,
			expected: 0x0101010101000101,
		},
	}

	for _, tc := range testCases {
		t.Run(strconv.Itoa(int(tc.word)), func(t *testing.T) {
			if ZeroByte(tc.word, tc.idx) != tc.expected {
				t.Errorf("unexpected result: %x", ZeroByte(tc.word, tc.idx))
			}
		})
	}
}
